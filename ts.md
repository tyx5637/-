...
1. 泛型可以理解为变量
   ...
   ![1667993054805](image/ts/1667993054805.png)
2. Partial的实现
...
3. 在ts中，泛型参数存在默认约束，这个月数值在TS 3.9版本以前是any，3.9版本以后是unknown
4. 多泛型参数其实就像接受更多参数的函数，其内部的运行逻辑会更加抽象，表现在参数需要进行的逻辑运算会更加复杂![1667992089162](image/ts/1667992089162.png)
5. tsx文件中泛型的<>尖括号可能会造成报错，我们需要使其长得更像泛型![1667993337173](image/ts/1667993337173.png)

## 结构化类型系统

1. 鸭子类型：这个名字来源于鸭子测试，核心理念为如果A是M，A和B具有相同的属性则判断B也是M![1668057739458](image/ts/1668057739458.png)
2. 标称类型系统：标称类型系统要求，两个可兼容的类型，其名称必须是完全一致的。对于标称类型系统，父子类型关系只能通过显示的继承来实现，称为标称子类型。![1668058085240](image/ts/1668058085240.png)

   ![1668058140968](image/ts/1668058140968.png)
3. 类型，类型系统与类型检查：
...
**never <** **字面量类型 < 包含此字面量类型的联合类型（同一基础类型） < 对应的原始类型 < 原始类型对应的装箱类型 < Object 类型 < any/known![1668062118952](image/ts/1668062118952.png)**

- 对于基类和派生类，通常情况下派生类会完全保留基类的结构，而只是自己新增新的属性与方法，在结构化类型的比较下，其类型自然会存在子类型关系。更不用说派生类本身就是 extends 基类得到的。
- 对于联合类型的比较，我们只需要比较一个联合类型是否可以被视为另一个联合类型的子集，即这个联合类型中所有成员在另一个联合类型中都能找到

  ![1668062580347](image/ts/1668062580347.png)

## 条件类型与infer(目前有点看不太懂)

1. infer关键字：infer是inference的缩写，infer R中R表示待推断的类型。infer只能在条件类型中使用![1668069682674](image/ts/1668069682674.png)

## 内置工具类型基础

1. 工具类型的分类：

   - 属性修饰工具类型：对对象属性和数组元素的可选/必选，只读/可写
...
3. 结构工具类型：
   ...
   - 结构声明：快速声明一个结构，比如Record,我们常使用 `Record<string, unknown>`和 `Record<string, any>`代替object
    ...
    - 结构处理工具类型：Pick和Omit。Pick是保留这些传入的键，Omit是移除这些传入的键
...
4. 集合工具类型
   ...
   - 交集: `type Extract<T,U> = T extends U ? T : never`;
   - 差集: `type Exclude<T,U> = T extends U ? never : T`;
   - 并集: `type Concurrence<A, B> = A | B;`
   - 补集: `type Complement<A, B extends A> = Exclude<A, B>`

## 上下文类型

上下文类型的核心理念：基于位置的类型推导，基于已定义的类型来规范开发者的使用
## 协变与逆变
协变：随着某一个量的变化随之变化一致的即称为协变
逆变：随着某一个量的变化随着变化相反的即称为逆变
## 在React中愉快的使用Ts：内置类型和泛型坑位
### 组件声明：我们声明一个 React 组件的方式
### 泛型坑位：React API 中预留出的泛型坑位
### 内置类型定义
